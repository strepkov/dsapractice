...
%\begin{itemize}
	%\item Use binary mode to read from acceleration sensor to avoid parsing the data manually as a string to simplify the code.
	%\item Calibrate the acceleration sensor threshold based on in-the-field testing (e.g. beside a highway) to make sure it emits minimal false positives (theft detected although the vehicle is in fact not stolen) while still reporting all true positives.
	%\item Test all branches of the algorithm, ideally with a real vehicle data in order to ensure the correctness of the algorithm and the implementation.
%\end{itemize}
